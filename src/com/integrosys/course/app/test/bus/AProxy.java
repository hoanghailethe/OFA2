 ...
 */
public class AProxy {
    HashMap<String, OBA> store = new HashMap();
    ...
    public OBA get(String key) throws AException {
        DefaultLogger.debug(AHelper.class, "OBA id - GET (from AProxy)" + key);
...
//        return makeTrxObject(key.hashCode());
    ...
    public List<OBA> findByName(String key) {
        return store.values().stream().filter(p -> p.getName().contains(key)).collect(Collectors.toList());
    }

    public Collection findById(String key) {
        return store.values().stream().filter(p -> Long.toString(p.getId()).equals(key)).collect(Collectors.toList());
    }

    public Collection findByCountry(String key) {
        return store.values().stream().filter(p-> p.getCountry().equals(key)).collect(Collectors.toList());
    }

    public Collection findByRole(String key) {
        return store.values().stream().filter(p -> p.getRole().equals(key)).collect(Collectors.toList());
    }

    public Collection findByRights(String key) {
        return store.values().stream().filter(p -> p.getRights().contains(key)).collect(Collectors.toList());
    }
}
